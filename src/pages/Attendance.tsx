...
const rangePresets: Array<{ value: RangePreset; label: string; days: number | null }> = [
...
const isDateWithinWindow = (dateKey: string, rangeWindow: RangeWindow | null) => {
  if (!rangeWindow) {
    return true;
  }
  ...
  return Array.from(byDate.entries())
    ...
    .map(([dateKey, present]) => ({
      day: new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric" }).format(
        parseDateKey(dateKey) ?? new Date(`${dateKey}T00:00:00`),
      ),
      present,
      absent: 0,
    }));
...
const buildTrendFromDaily = (dailyComparison: AttendanceAnalytics["dailyComparison"]) =>
...
const buildPunctualityFromRecords = (records: AttendanceAnalytics["records"]) => {
...
const Attendance = () => {
  ...
  const analyticsForCharts = useMemo(() => {
    ...
    const safeDailyComparison =
      dailyComparison.length > 0 ? dailyComparison : attendanceData.dailyComparison;
  ...
  const computedSummary = useMemo(() => {
    ...
    const hasHeatmapCoverage = Boolean(filteredHeatmap && filteredHeatmap.dates.length > 0);
  ...
  const kpis = useMemo(() => {
    ...
    const presentRate = observedDays > 0 ? (totals.present / observedDays) * 100 : 0;
    const lateRate = totals.present > 0 ? (totals.late / totals.present) * 100 : 0;
    const absenceRate = observedDays > 0 ? (totals.absent / observedDays) * 100 : 0;
    const avgClockIn = checkInSamples > 0 ? Math.round(totalCheckInMinutes / checkInSamples) : null;
  ...
  }, [computedSummary, filteredRecords]);

  const filteredSummaryRows = useMemo(() => {
    const query = summarySearch.trim().toLowerCase();

    return computedSummary.filter((row) => {
  ...
  }, [computedSummary, summaryFilter, summarySearch]);
  ...
  const selectedEmployeeSummary = useMemo(
    () =>
      selectedEmployeeKey
        ? computedSummary.find((row) => summaryKey(row) === selectedEmployeeKey) ?? null
        : null,
    ...
    <section className="space-y-6">
      <section className="sticky top-16 z-20 rounded-2xl border border-slate-200/80 bg-white/95 px-5 py-5 shadow-soft backdrop-blur sm:px-6">
        <div className="grid gap-5 xl:grid-cols-[minmax(280px,1fr)_minmax(540px,auto)] xl:items-start">
          <div className="max-w-2xl space-y-2.5">
            ...
            {importInfo ? (
              <p className="text-xs font-medium text-emerald-700">{importInfo}</p>
            ) : null}
          </div>
      ...
      <section className="card-surface overflow-hidden">
        <div className="border-b border-slate-200/80 px-4 py-4 sm:px-5">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Search and filter employees, then click a row to drill into individual records.
          </p>
        </div>
        ...
        {filteredSummaryRows.length > 0 ? (
            ...
            <table className="min-w-full divide-y divide-slate-200/80 text-sm">
              <thead className="bg-slate-50/80">
                <tr>
                  ...
                  <th className="px-5 py-3 text-right font-semibold text-slate-600">Avg Clock-In</th>
                  <th className="px-5 py-3 text-right font-semibold text-slate-600">Present</th>
                  <th className="px-5 py-3 text-right font-semibold text-slate-600">On-Time</th>
                  <th className="px-5 py-3 text-right font-semibold text-slate-600">Late</th>
                  <th className="px-5 py-3 text-right font-semibold text-slate-600">Absent</th>
                </tr>
              </thead>
          ...
          <div className="px-5 py-8 text-center">
            <p className="text-sm font-medium text-slate-600">No employees match this filter.</p>
            <p className="mt-1 text-xs text-slate-500">
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-200/80 px-5 py-4">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Individual attendance record {rangeWindow ? `(${rangeWindow.label})` : ""}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={selectedEmployeeIndex < 0 || modalEmployeeList.length <= 1}
              ...
              <div className="border-b border-slate-200/80 px-5 py-4">
                <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                  <div className="rounded-lg bg-slate-50 px-3 py-2">
                    ...
                    <p className="mt-1 text-lg font-semibold text-emerald-700">{selectedEmployeeSummary.daysPresent}</p>
                  </div>
                  <div className="rounded-lg bg-slate-50 px-3 py-2">
                    <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">On-Time</p>
                    <p className="mt-1 text-lg font-semibold text-emerald-700">
                      {Math.max(selectedEmployeeSummary.daysPresent - selectedEmployeeSummary.lateClockIns, 0)}
                    </p>
                  </div>
                  <div className="rounded-lg bg-slate-50 px-3 py-2">
                    <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Late</p>
                    <p className="mt-1 text-lg font-semibold text-amber-700">{selectedEmployeeSummary.lateClockIns}</p>
                  </div>
                  <div className="rounded-lg bg-slate-50 px-3 py-2">
                    <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Absent</p>
                    <p className="mt-1 text-lg font-semibold text-rose-700">{selectedEmployeeSummary.daysAbsent}</p>
                  </div>
                </div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {selectedTimeline.map((entry) => (
                      <span
                        ...
                        className={`h-2.5 w-4 rounded-sm ${timelineStatusClass[entry.status]}`}
